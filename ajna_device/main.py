...
if object_choice in file_paths:
    keypress_thread = threading.Thread(target=stop_speech)
    keypress_thread.daemon = True  # Set the thread as a daemon so it stops when the main program exits
    keypress_thread.start()
    start1(file_paths[object_choice])
else:
